...
class IsiRPS_isi extends CI_Controller {
	...
	{
		$this->load->view('isirps_isi');
	}
	function rprps($idrps){
		$this->load->model("misirps");
		$datas=$this->misirps->GetRPRPS("where rps.id_rps='$idrps'");
		$data["datarprps"] = $datas;
		$this->load->model("misirps");
		$data2=$this->misirps->GetDataRPS("where rps.id_rps=$idrps");
		$data["datarps"] = $data2;
		$kodemakul=$data2[0]['kode_matkul'];
		$data1=$this->misirps->GetRPSOLD("rps.kode_matkul='$kodemakul'");
		$data["rpsold"] = $data1;
		...
		$data["datar"] = $data7;
		$data8=$this->misirps->GetTahun("where rps.id_rps=$idrps");
		$data["tahun"] = $data8;
		$this->load->view('isirps_isi', $data);
	}
